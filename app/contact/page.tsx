  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!formData.name.trim() || !formData.email.trim() || !formData.subject.trim() || !formData.message.trim()) {
        setStatusMessage("กรุณากรอกข้อมูลให้ครบก่อนส่งข้อความ")
        return
      }

      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) {
        setStatusMessage("รูปแบบอีเมลไม่ถูกต้อง")
        return
      }

      await new Promise((resolve) => setTimeout(resolve, 700))

      setStatusMessage("ส่งข้อความสำเร็จแล้ว เราจะติดต่อกลับให้เร็วที่สุด")
      setFormData({ name: "", email: "", subject: "", message: "" })
    } catch (error) {
      setStatusMessage("ส่งข้อความไม่สำเร็จ กรุณาลองใหม่อีกครั้ง")
    } finally {
    ...
    <div className="min-h-screen bg-[#0a0b0f] text-white selection:bg-[#228dbd]/30">
      ...
      <main className="pt-32 pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          >
            Contact <span className="text-[#228dbd] text-neon-glow-brand">FARDARCLOUD</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {statusMessage && (
                <p className="rounded-2xl border border-[#228dbd]/20 bg-[#228dbd]/10 px-4 py-3 text-sm text-[#bfe7ff]">
                  {statusMessage}
                </p>
              )}
              ...
              <button
                ...
                className="w-full bg-[#228dbd] hover:bg-[#1a6e94] text-white py-4 rounded-2xl font-bold transition-all shadow-[0_0_30px_rgba(34,141,189,0.2)] hover:shadow-[0_0_40px_rgba(34,141,189,0.4)] flex items-center justify-center gap-2 group disabled:cursor-not-allowed disabled:opacity-70"
              >
