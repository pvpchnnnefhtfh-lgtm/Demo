...
export function useCurrency(): UseCurrencyReturn {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        const { rates, timestamp } = JSON.parse(stored) as {
          rates?: Record<string, number>;
          timestamp?: number;
        };

        const dayInMs = 24 * 60 * 60 * 1000;
        if (rates && timestamp && Date.now() - timestamp < dayInMs) {
